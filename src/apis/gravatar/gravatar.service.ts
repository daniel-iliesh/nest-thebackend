...
export class GravatarService {
    ...
    constructor(
        @Inject(CustomModule.GRAVATAR_INSTANCE) private readonly gravatarApi: AxiosInstance,
    ...
    async getProfile(): Promise<GravatarProfile | undefined> {
        try {
            const { data } = await this.gravatarApi.get<GravatarProfile>(`/profiles/${this.stub}`);
            return data;
        } catch (error) {
